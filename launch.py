...
def launch():
    ...
    while is_playing_game:
        try:
            player_balance = int(input("For how much money you want to buy chips: $"))
            break
        except:
    ...
    while is_playing_game:
        ...
        round = Black_Jack(bet, deck)

        is_black_jack = round.check_black_jack(round.player_value, round.dealer_value)
        ...
        while True:
            get_card = input(
                "Do you want to take another card (Yes/No): \n> ").lower()
        ...
        print(f"Your current balance is: {player_balance}")
        if player_balance == 0:
            deposit_more = input("You are out of money, do you want to deposit some more? (Yes/No) \n> ").lower()
            if deposit_more == 'yes':
                while True:
                    try:
                        player_balance = int(input("For how much money you want to buy chips: $"))
